...
namespace Diagram.Logic
{
    ...
    {
        public static IEnumerable<(INode, IEnumerable<ChildDiagramConnector>)> ChildConnections(this Dictionary<object, DiagramConnectorNode> logic, IList<INode> parents, IConnectorConverter converter)
        {
            foreach (INode person in parents)
                yield return (person, ChildConnections(logic, person, converter));
        }
