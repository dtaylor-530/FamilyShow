...
namespace Microsoft.FamilyShowLib
{
  ...
  class FamilyMap : Dictionary<string, Family>
  {
    ...
    /// </summary>
    //public void Create(IEnumerable<INode> people)
    //{
    //  Clear();
    ...
    //  // more children.
    //  foreach (INode person in people)
    //  {
    //    var parents = person.Parents.ToList();
    //    if (parents.Count > 0)
    //    {
    //      // Only using the first two parents.
    //      var parentLeft = parents.First();
    //      var parentRight = (parents.Count > 1) ? parents[1] : null;
    ...
    //  {
    //    IEnumerable<INode> spouses = person.Spouses;
